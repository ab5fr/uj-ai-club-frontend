 ...
 * @param {'json'|'blob'|'none'} [options.responseType='json']
 */
...
export const challengesApi = {
  ...
  closeSession: async (challengeId) => {
    return apiRequest(`/challenges/${challengeId}/close-session`, {
      method: "POST",
    });
  },
...
export const adminNotebooksApi = {
  ...
  getFormgraderUrl: async () => {
    return apiRequest("/admin/formgrader/url");
  },
};
...
export const adminSubmissionsApi = {
  ...
  getFileBlob: async (submissionId, download = false) => {
    return apiRequest(
      `/admin/submissions/${submissionId}/file${download ? "?download=1" : ""}`,
      { responseType: "blob" },
    );
  },
  ...
  remove: async (submissionId) => {
    return apiRequest(`/admin/submissions/${submissionId}`, {
      method: "DELETE",
    });
  },

  grantAttempts: async (submissionId, extraAttempts) => {
    return apiRequest(`/admin/submissions/${submissionId}/grant-attempts`, {
      method: "POST",
      body: JSON.stringify({ extraAttempts }),
    });
  },
};
