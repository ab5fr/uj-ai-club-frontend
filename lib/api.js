...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://api.aiclub-uj.com/";
...
async function apiRequest(endpoint, options = {}) {
  ...
  // Allow callers to override credentials mode (include, same-origin, omit)
  const credentialsMode = options.credentials ?? "include";
  const config = {
    ...options,
    // Default to include, but allow override via options.credentials
...
export const leaderboardApi = {
  // Public endpoint - credentials not required and cause CORS issues when included
  getAll: async () => {
    return apiRequest("/leaderboards", { credentials: "omit" });
  },
};
...
export const resourcesApi = {
  getAll: async () => {
    return apiRequest("/resources", { credentials: "omit" });
  },

  getById: async (id) => {
    return apiRequest(`/resources/${id}`, { credentials: "omit" });
  },
};
